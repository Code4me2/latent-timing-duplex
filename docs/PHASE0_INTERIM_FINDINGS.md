...
|---|---|---|---|
| DuplexChat EN reconstruct | 5 episodes / 10 clips | 1383.44 s (~23.1 min) | Stereo 24 kHz DialogueSidon reconstruct |
...
Uncertainty: conversation/episode-level bootstrap, B=10000, seed `20260903`, Efron 2.5/97.5 percentiles. DuplexChat primary unit = episode (n=5); within-episode metrics duration-weighted before bootstrap. CANDOR primary unit = conversation UUID (n=20).
...
BayLing token NLL / perplexity uses a different vocabulary and objective. Report only as a separate frozen baseline: DuplexChat dw NLL ~7.17 (PPL ~1297); CANDOR20 dw NLL ~8.80 (PPL ~6667). Do **not** rank Moshi vs BayLing on these numbers.
...
3. **Cross-corpus contrast survives matching CIs.** DuplexChat episode CIs for Moshi audio NLL lie entirely below the CANDOR20 point estimate; DuplexChat `p(shift)` CIs lie entirely above CANDOR20. Intervals do not overlap on either contrast.
4. **Caveats.** DuplexChat n=5 episodes is small and CIs are wide; corpora differ in genre, length, reconstruction path, and sample rate (24 kHz vs 48 kHz). Channel packing passed automated spot-check but remains a documented convention (especially DuplexChat SPEAKER_XX unbound to L/R). This is evidence of a measurable domain/timing difference, **not** confirmation of a latent predictive timing objective.
...
- DuplexChat: `/home/velvet/cs199-duplexchat-work/` (`duplexchat10_summary.json`, `bootstrap_ci_duplexchat10.py`)
...
2. Expand DuplexChat English reconstruct for tighter episode-level CIs (in progress).
3. Declare a larger matched evaluation protocol (same duration bins / same channel rules) before changing models.
