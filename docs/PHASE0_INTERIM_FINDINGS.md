...
4. **Caveats.** DuplexChat10 n=5 episodes is small and CIs are wide; expanded n=18 still has a wide `p(shift)` interval. Corpora differ in genre, length, reconstruction path, and sample rate (CANDOR 48 kHz vs DuplexChat 24 kHz). Channel packing passed automated spot-check but remains a documented convention (especially DuplexChat SPEAKER_XX unbound to L/R). This is evidence of a measurable domain/timing difference, **not** confirmation of a latent predictive timing objective. Sample-rate confound is the next controlled check before Phase 1.

## Artifact paths (spark-61dd)
- CANDOR: `/home/velvet/cs199-candor-work/` (`nll_candor20_moshi.jsonl`, `nll_candor20_bayling.jsonl`, `vap_candor20.jsonl`, `candor20_summary.json`, `candor_rest_summary.json`, `candor_part001_pooled_summary.json`, `nll_candor25_*.jsonl`, `vap_candor25.jsonl`, `bootstrap_ci.py`)
...
4. Controlled sample-rate check (CANDOR 48 kHz vs DuplexChat 24 kHz) and a matched evaluation protocol (same duration bins / same channel rules) before changing models.
5. Only then start latent predictive objective training (Phase 1).
...
- Sample-rate confound check (CANDOR 48 kHz vs DuplexChat 24 kHz) / matched protocol
