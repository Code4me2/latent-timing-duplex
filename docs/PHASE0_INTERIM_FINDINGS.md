...
Moshi audio CIs still fail to overlap (barely); the gap slightly widens. VAP `p(shift)` still overlaps. Next for true equal support: fixed-length first-W-seconds Moshi NLL.
...
5. **Duration matching does not close the Moshi gap.** Supports are DISJOINT (CANDOR min 1538 s > DC max 904 s). On the 18 shortest CANDOR (median 1681 s vs DC 230 s), Moshi audio dw 0.625 [0.605, 0.644] vs DC 0.549 [0.483, 0.605]; CIs still fail to overlap (barely) and the gap slightly widens. VAP `p(shift)` still overlaps: matched 0.484 [0.435, 0.532] vs DC 0.506 [0.408, 0.608].
6. **Caveats.** DuplexChat10 n=5 episodes is small and CIs are wide; expanded n=18 still has a wide `p(shift)` interval. Duration NN cannot equalize support (sensitivity window empty). Corpora still differ in genre and reconstruction path. Channel packing passed automated spot-check but remains a documented convention (especially DuplexChat SPEAKER_XX unbound to L/R). This is evidence of a measurable domain/timing difference, **not** confirmation of a latent predictive timing objective. Next controlled check is fixed-length first-W-seconds Moshi NLL for true equal support.

## Artifact paths (spark-61dd)
- CANDOR: `/home/velvet/cs199-candor-work/` (`nll_candor20_moshi.jsonl`, `nll_candor20_bayling.jsonl`, `vap_candor20.jsonl`, `candor20_summary.json`, `candor_rest_summary.json`, `candor_part001_pooled_summary.json`, `nll_candor25_*.jsonl`, `vap_candor25.jsonl`, `candor20_24khz_summary.json`, `nll_candor20_24khz_moshi.jsonl`, `duration_matched_summary.json`, `duration_matched_summary.md`, `duration_matched_pairs.json`, `bootstrap_ci.py`)
...
5. ~~Duration-matched reanalysis~~ **done** (supports DISJOINT; Moshi gap holds on 18 shortest CANDOR; VAP overlaps). Matched protocol filed in `docs/EVAL_PROTOCOL_PHASE0.md`.
6. Fixed-length first-W-seconds Moshi NLL for true equal support.
7. Only then start latent predictive objective training (Phase 1).
...
- Fixed-length first-W-seconds Moshi NLL (true equal support)
